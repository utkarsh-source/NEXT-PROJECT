    ...
    return (
        <div className="bg-gray-50 fixed w-screen h-screen top-0 left-0 bottom-0 right-0 flex items-center justify-center">
            <form  className="relative w-11/12 md:w-8/12 lg:w-5/12" onSubmit={handleFormSubmit}>
                ...
                <button type="submit" onClick={()=> setUsingGoogle(false)} className="flex items-center justify-center w-full mb-3 btn-indigo"><span className="absolute grid place-items-center left-4"><Image src={lock} alt="Google" width={30} height={30} /></span>   {user.hasAccount ? "Sign In" : "Create Account"}</button>
