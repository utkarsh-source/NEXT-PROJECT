    ...
    return (
        <div className="bg-gray-50 fixed w-screen h-screen top-0 left-0 bottom-0 right-0 flex items-center justify-center">
            <form  className="relative w-5/12" onSubmit={handleFormSubmit}>
                ...
                <button type="submit" onClick={()=> setUsingGoogle(false)} class="flex items-center justify-center w-full mb-3 btn-indigo"><span className="absolute grid place-items-center left-4"><Image src={lock} alt="Google" width={25} height={25} /></span>   {user.hasAccount ? "Sign In" : "Create Account"}</button>
                <button type="submit" onClick={()=> setUsingGoogle(true)} class="flex items-center justify-center  w-full plain-btn"><span className="grid place-items-center absolute left-4"><Image src={google} alt="Google" width={25} height={25} /> </span> Sign in with Google</button>
            </form>
